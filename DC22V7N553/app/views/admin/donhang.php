                ...
                <div class="content-wrapper">
                    <div class="d-flex justify-content-between align-items-center mb-3 toolbar">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <h2 class="mb-0"><i class="bi bi-cart-check"></i> Quản lý đơn hàng</h2>
                        </div>
                        <div>
                        ...
                        <div class="card-body">
                            <form method="GET" class="d-flex gap-2 flex-wrap">
                                ...
                                <?php if (!empty($search) || !empty($trang_thai_filter)): ?>
                                    <a href="index.php?page=admin_donhang" class="btn btn-outline-secondary"><i class="bi bi-x"></i> Xóa bộ lọc</a>
                                <?php endif; ?>
                            </form>
                        </div>
                    </div>
                            ...
                            <?php if ($totalPages > 1): ?>
                                <nav aria-label="Page navigation" class="mt-3">
                                    <ul class="pagination justify-content-center">
                                        <?php if ($page > 1): ?>
                                            <li class="page-item">
                                                <a class="page-link" href="?page=admin_donhang&p=<?= $page - 1 ?>&search=<?= urlencode($search) ?>&trang_thai=<?= urlencode($trang_thai_filter) ?>">Trước</a>
                                            </li>
                                        <?php endif; ?>
                                        <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
                                            <li class="page-item <?= $i === $page ? 'active' : '' ?>">
                                                <a class="page-link" href="?page=admin_donhang&p=<?= $i ?>&search=<?= urlencode($search) ?>&trang_thai=<?= urlencode($trang_thai_filter) ?>"><?= $i ?></a>
                                            </li>
                                        <?php endfor; ?>
                                        <?php if ($page < $totalPages): ?>
                                            <li class="page-item">
                                                <a class="page-link" href="?page=admin_donhang&p=<?= $page + 1 ?>&search=<?= urlencode($search) ?>&trang_thai=<?= urlencode($trang_thai_filter) ?>">Sau</a>
                                            </li>
                                        <?php endif; ?>
                                    </ul>
                                </nav>
